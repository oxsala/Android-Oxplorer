...
public class ZoomControl implements Observer {
	...
	/*
	 * public ZoomControl() { mPanDynamicsX.setFriction(2f);
	 * mPanDynamicsY.setFriction(2f); mPanDynamicsX.setSpring(50f, 1f);
	...
	public void startFling(float vx, float vy) {
		...
		mPanDynamicsX.setState(mState.getPanX(), vx
				/ mState.getZoomX(aspectQuotient), now);
		mPanDynamicsY.setState(mState.getPanY(), vy
				/ mState.getZoomY(aspectQuotient), now);
